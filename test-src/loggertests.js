(function () {
  ...
  QUnit.test(
    "Global Logger - Log messages routed to logger function",
    function (assert) {
  ...
  QUnit.test(
    "Global Logger - Timing operations routed to logger function",
    function (assert) {
      var logger = this.logger;
  ...
  QUnit.test("Named logger messages not routed to global logger", function (
    assert
  ) {
    var logger = this.logger;
    ...
    assert.ok(
      this.calls.length === 1,
      "Log message was only routed via named logger."
    );
  });

  QUnit.test("Named logger, same instance returned", function (assert) {
    assert.strictEqual(
      this.logger.get("myLogger"),
      this.logger.get("myLogger"),
      "Exact same logger instance returned"
    );
  });

  QUnit.test("Named logger setLevel does not affect global logger", function (
    assert
  ) {
  ...
  QUnit.test("Logger.log convenience method", function (assert) {
    ...
    assert.strictEqual(
      this.calls[0].context.level,
      Logger.INFO,
      "Logger.log message routed at INFO level"
    );
  });
  ...
  QUnit.test(
    "Logger.setLevel - Modify log filter level of all named loggers",
    function (assert) {
      var logger = this.logger;
      ...
      assert.ok(
        this.calls.length === 1,
        "Logger.setLevel() sets log filter level for all named loggers"
      );
    }
  );
  ...
  QUnit.test("Logger.useDefaults logs to console (except trace)", function (
    assert
  ) {
    var logger = this.logger;
    ...
    assert.ok(
      ...
      "logger.trace calls console.trace"
    );
  ...
  QUnit.test(
    "Logger.useDefaults forward timing operations through to the console",
    function (assert) {
      var logger = this.logger;
      ...
      assert.ok(
        console.timeEnd.calledOnce,
        "logger.timeEnd calls console.timeEnd"
      );
  ...
  QUnit.test(
    "Logger.useDefaults can be supplied a custom message formatter",
    function (assert) {
      var sandbox = sinon.sandbox.create();
      sandbox.stub(console, "warn");
      ...
      assert.deepEqual(
        formatterSpy.firstCall.args[0],
        ["Hello", "World"],
        "Log messages supplied to handler"
      );
      assert.ok(
        formatterSpy.firstCall.args[1].name === "Dave",
  ...
  QUnit.test("Logger.setDefaults is an alias for useDefaults", function (
    assert
  ) {
    var sandbox = sinon.sandbox.create();
    sandbox.stub(console, "warn");
